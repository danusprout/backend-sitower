...
async function main() {
  ...
  try {
    ...
    console.log('🧹 Clearing existing SKTT towers and routes...');
    ...
    for (const row of rows) {
      ...
      const idBase = normalizeId(ruas);
      const bayNumMatch = bayPenghantar.match(/#(\d+)/);
      const bayNum = bayNumMatch ? bayNumMatch[1] : normalizeId(bayPenghantar).slice(-4);
      const id = `SKTT-${idBase}-${bayNum}`;
      const nama = ruas;
      ...
      await client.query(
        `INSERT INTO "Tower" (id, nama, lat, lng, tegangan, tipe, kondisi, lokasi, jalur, "createdAt", "updatedAt", "statusKerawanan")
         ...
         ON CONFLICT (id) DO UPDATE SET nama = EXCLUDED.nama, lat = EXCLUDED.lat, lng = EXCLUDED.lng, tegangan = EXCLUDED.tegangan, lokasi = EXCLUDED.lokasi, jalur = EXCLUDED.jalur, "updatedAt" = NOW()`,
